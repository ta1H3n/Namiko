    ...
    public class Currency : CustomModuleBase<ICustomContext>
    {
        ...
        public async Task DailyCmd()
        {
            ...
            if ((daily.Date + ms) < timeNow)
            {
                ...
                await ReplyAsync(dayslate == 0 ? "" : $"You are **{dayslate}** days late to claim your daily. For every day missed, you lose 25% of your streak.", false, CurrencyUtil.DailyGetEmbed(Context.User, daily.Streak, amount, BalanceDb.GetToasties(Context.User.Id, Context.Guild.Id), TextCommandService.GetPrefix(Context)).Build());
            }
            else
            {
                ...
                await ReplyAsync("", false, CurrencyUtil.DailyWaitEmbed(Context.User, hours, minutes, seconds, TextCommandService.GetPrefix(Context)).Build());
            }
        }
        ...
        public async Task Weekly()
        {
            ...
            if (weekly.Date.AddHours(hours).CompareTo(DateTime.Now) < 0)
            {
                ...
                await ReplyAsync(text, false, CurrencyUtil.WeeklyGetEmbed(amount, BalanceDb.GetToasties(Context.User.Id, Context.Guild.Id), Context.User, TextCommandService.GetPrefix(Context)).Build());
                return;
            }

            await ReplyAsync("", false, CurrencyUtil.WeeklyWaitEmbed(weekly.Date.AddHours(hours), Context.User, TextCommandService.GetPrefix(Context)).Build());
        }
        ...
        public async Task BuyLootbox()
        {
            ...
            await ReplyAsync(embed: new EmbedBuilderPrepared(Context.User)
                .WithDescription($"You bought {amount}x **{box.Name}**!\nType `{TextCommandService.GetPrefix(Context)}open` to open it!")
                .Build());
        }
