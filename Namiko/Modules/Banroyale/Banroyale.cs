...
namespace Namiko
{
    ...
    public class Banroyales : CustomModuleBase<ICustomContext>
    {
        ...
        public async Task NewBanroyale([Description("Only allow members with this role to join")][Remainder] IRole role = null)
        {
            ...
            if (banroyale != null)
            {
                await ReplyAsync($":x: There is already a running Ban Royale in this channel. Type `{TextCommandService.GetPrefix(Context)}cbrl` to cancel it.");
                return;
            }
        ...
        public async Task Banroyale()
        {
            ...
            if (banroyale == null)
            {
                await ReplyAsync($"There is no running Ban Royale in this channel. `{TextCommandService.GetPrefix(Context)}nbrl` to start a new one.");
                return;
            }
